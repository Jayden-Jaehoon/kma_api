# ASOS (Automated Surface Observing System) 기상청 지상관측 API 파이프라인

기상청 API(ASOS)에서 연도별 일자료를 다운로드하고 전처리하여 CSV로 저장하는 간단한 파이프라인입니다. 또한 관측소(지점) 정보를 내려받는 도구도 포함합니다.
...
실행 시
- `asos/data/raw_data/`에 연도별 원본 텍스트가 저장되고,
- `asos/data/post_process_data/`에 연도별 CSV가 생성됩니다.
