...
격자(위경도) 좌표 → 법정동(읍면동) 코드 매핑 모듈 (UMD 버전)
...
기존 geocode.py와의 차이점
--------------------------
1. 행정동(ADM) → 법정동(UMD, 읍면동) 경계 사용
2. 전국 17개 시도별로 분리된 shapefile을 통합 로드
3. 컬럼명: ADM_CD/ADM_NM → EMD_CD/EMD_NM
...
from .config_v2 import FusionConfigV2


class GridToEmdUmdMapper:
    """격자 좌표를 법정동(읍면동) 코드로 매핑하는 클래스 (UMD 버전).

    이 클래스는 크게 두 가지 리소스를 다룹니다.

    - 격자 위경도: `FusionConfigV2.grid_latlon_nc` (NetCDF)
    - 법정동 폴리곤: `FusionConfigV2.geodata_umd_dir` 하위의 시도별 Shapefile

    결과로 생성되는 매핑 테이블은 `FusionConfigV2.grid_mapping_file_umd`(Parquet)에 저장됩니다.
    ...
    def __init__(self, config: Optional[FusionConfigV2] = None):
        from .config_v2 import DEFAULT_CONFIG_V2
        self.config = config or DEFAULT_CONFIG_V2
    ...
    def build_mapping(self, force_rebuild: bool = False) -> pd.DataFrame:
        ...
        """
        mapping_path = self.config.grid_mapping_file_umd
    ...
    def load_mapping(self) -> pd.DataFrame:
        ...
        - 없으면 `FusionConfigV2.grid_mapping_file_umd`(Parquet)을 읽어 캐시에 저장 후 반환
...
def build_grid_mapping_umd(config: Optional[FusionConfigV2] = None, force: bool = False) -> pd.DataFrame:
    """격자-법정동(읍면동) 매핑 테이블 생성 (편의 함수).

    스크립트/노트북 등에서 클래스를 직접 만들지 않고도 한 줄로 실행하기 위한 래퍼입니다.

    Args:
        config: `FusionConfigV2` (생략 시 `DEFAULT_CONFIG_V2` 사용)
        force: True면 기존 매핑 파일이 있어도 재생성
    """
    mapper = GridToEmdUmdMapper(config)
    return mapper.build_mapping(force_rebuild=force)


if __name__ == "__main__":
    # 테스트 실행
    mapper = GridToEmdUmdMapper()
